...
Video: n8n Now Runs My ENTIRE Homelab Part 2: Building Terry - Your AI IT Employee
...
Add SSH node → Execute a command
Convert to subworkflow: Hover over node → Three dots → "Convert node to subworkflow"
Edit the subworkflow:
...
Set interval: Every 5 minutes (or your preference)
Add an Edit Fields node between Schedule and AI Agent
Set two fields:
...
Click "Add option" → "Require specific output format"
Add Structured Output Parser node
Configure the JSON schema:
{
  "website_up": true,
  "message": "message"
}
...
Set Chat ID
Map the message text from Terry's output
...
Add Edit Fields node between Schedule and AI Agent
Set prompt field with your monitoring instruction
Set chatId field with unique identifier
Update AI Agent to use these fields instead of Chat Trigger
Update Simple Memory to use {{ $json.chatId }}
Too Many Iterations Error
Issue: Terry is running too many tool calls.

Solution:

Increase max iterations in AI Agent settings (default is 10)
Make prompts more specific to reduce trial-and-error
Upgrade to smarter model (GPT-4 instead of GPT-4-mini)
Terry Isn't Asking for Approval
Issue: Permission rules not clear in prompt.

Solution:

Add explicit "CRITICAL PERMISSION RULES" section
Provide clear examples of commands requiring approval
Add structured output fields: needs_approval, commands_requested
...
Test with "Execute node" to verify connection
SSH Commands Failing
Issue: Subworkflow not configured correctly.

Solution:

Verify SSH credentials in subworkflow
Ensure Start node has command field defined
Check SSH node is mapping the command variable
Test with simple command like hostname
Best Practices
Start Simple: Begin with basic monitoring, then add complexity
Document Everything: Keep notes of what Terry can access
Test in Sandbox: Use the demo website setup before connecting to production
Use Human-in-the-Loop: Always require approval for system-modifying commands
Monitor Terry: Set up alerts for when Terry takes actions
Iterate Prompts: Refine system prompts based on Terry's behavior
Version Control: Save different versions of your workflows
Security First: Use Twingate or VPN for remote access, never expose directly
What's Next?
